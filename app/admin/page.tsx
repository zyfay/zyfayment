...
export default function AdminDashboard() {
  ...
  const load = async (t?: string) => {
    ...
    if (res.unauthorized) { sessionStorage.removeItem("admin_token"); router.replace("/admin/login"); return; }
  ...
  const simulatePayment = async (trx: Transaction) => {
    ...
    const res = await fetch("/api/simulate", {
      ...
      body: JSON.stringify({ orderId: trx.orderId, amount: trx.amount, adminKey: token }),
    }).then((r) => r.json());
  ...
  const confirmPayment = async (id: string) => {
    ...
    const res = await fetch("/api/webhook", {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ transactionId: id, adminKey: token }),
    }).then((r) => r.json());
  ...
  return (
    <div style={styles.page}>
      <aside style={styles.sidebar}>
        ...
        {isSandbox && <div style={styles.sandboxBadge}>🧪 SANDBOX MODE</div>}
              ...
              <table style={styles.table}>
                <thead>
                  <tr style={styles.thead}>
                    {["Order ID", "Pelanggan", "Email", "Jumlah", "Status", "Waktu", "Aksi"].map((h) => (
                ...
                <tbody>
                  {filtered.map((t) => (
                    <tr key={t.id} style={styles.tr}>
                      ...
                      <td style={styles.td}><span style={{ fontSize: 12, color: "#888" }}>{t.customerEmail}</span></td>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  logoutBtn: { width: "100%", padding: "9px", background: "#ff444420", color: "#ff6b6b", border: "1px solid #ff444430", borderRadius: 10, fontSize: 13, fontWeight: 600, cursor: "pointer" },
