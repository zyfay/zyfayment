...
function NotifPopup({userId,onClose,onUnreadChange}:{userId:string;onClose:()=>void;onUnreadChange:(n:number)=>void}) {
  ...
  useEffect(()=>{
    load();
    const ch=supabase.channel("notif-"+userId).on("postgres_changes",{event:"*",schema:"public",table:"notifications",filter:"user_id=eq."+userId},load).subscribe();
    ...
    <div style={{position:"fixed",inset:0,zIndex:100,background:"rgba(0,0,0,0.3)",backdropFilter:"blur(2px)"}} onClick={onClose}>
      <div style={{position:"absolute",top:"58px",right:"14px",width:"330px",background:"#111827",borderRadius:"16px",border:"1px solid rgba(255,255,255,0.09)",boxShadow:"0 20px 60px rgba(0,0,0,0.7)",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"13px 16px",borderBottom:"1px solid rgba(255,255,255,0.07)"}}>
          <div style={{display:"flex",alignItems:"center",gap:"7px"}}><IC.Bell s={15} c="#a5b4fc"/><span style={{fontWeight:700,fontSize:"13px",color:"#fff"}}>Pemberitahuan</span></div>
          <button onClick={onClose} style={{background:"rgba(255,255,255,0.07)",border:"none",borderRadius:"7px",width:"26px",height:"26px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><IC.X s={14} c="rgba(255,255,255,0.5)"/></button>
        </div>
        <div style={{maxHeight:"360px",overflowY:"auto",padding:"8px"}}>
          ...
          {!loading&&notifs.length===0&&(
            <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"10px",padding:"32px 16px"}}>
              <IC.Bell s={30} c="rgba(255,255,255,0.12)"/>
          ...
          {notifs.map(n=>(
            <div key={n.id} style={{borderRadius:"10px",padding:"11px 13px",marginBottom:"5px",borderLeft:n.read?"3px solid transparent":"3px solid #6c7ee1",background:n.read?"rgba(255,255,255,0.03)":"rgba(108,126,225,0.11)"}}>
              ...
              <div style={{display:"flex",gap:"6px",paddingLeft:n.read?"0":"13px"}}>
                {!n.read&&<button onClick={()=>markRead(n.id)} style={{display:"flex",alignItems:"center",gap:"5px",fontSize:"11px",padding:"5px 11px",borderRadius:"7px",border:"1px solid rgba(108,126,225,0.4)",background:"rgba(108,126,225,0.15)",color:"#a5b4fc",cursor:"pointer",fontWeight:600}}><IC.Check s={11} c="#a5b4fc"/>Dibaca</button>}
                <button onClick={()=>del(n.id)} style={{display:"flex",alignItems:"center",gap:"5px",fontSize:"11px",padding:"5px 11px",borderRadius:"7px",border:"1px solid rgba(239,68,68,0.3)",background:"rgba(239,68,68,0.1)",color:"#fca5a5",cursor:"pointer",fontWeight:600}}><IC.Trash s={11} c="#fca5a5"/>Hapus</button>
              </div>
            </div>
          ))}
        </div>
...
/* ── DOKUMENTASI ── */
const DOCS=[
  {id:"mulai",title:"Mulai Cepat",icon:"⚡",content:[
    {t:"p",v:"Zyfay Official adalah payment gateway Indonesia. Integrasikan dalam hitungan menit."},
    {t:"steps",v:["Daftar & buat akun di Zyfay Official","Generate API Key di menu API Settings","Simpan API Key, Merchant ID, dan Webhook Secret","Gunakan endpoint di bawah untuk membuat transaksi","Set Webhook URL untuk menerima notifikasi pembayaran"]},
    {t:"p",v:"Semua request menggunakan HTTPS. Gunakan Sandbox untuk testing sebelum go-live."},
  ]},
  {id:"auth",title:"Autentikasi",icon:"🔐",content:[
    {t:"p",v:"Setiap request ke API Zyfay Official wajib menyertakan header berikut:"},
    {t:"code",v:'Authorization: Bearer YOUR_API_KEY\nX-Merchant-ID: YOUR_MERCHANT_ID\nContent-Type: application/json'},
    {t:"p",v:"API Key bersifat rahasia. Jangan ekspos di frontend atau commit ke repository."},
    {t:"info",v:"Untuk keamanan ekstra, aktifkan IP Whitelist di API Settings agar hanya server tertentu yang bisa akses."},
  ]},
  {id:"buat",title:"Buat Transaksi",icon:"💳",content:[
    {t:"code",v:'POST https://api.zyfayofficial.com/v1/transactions\n\n{\n  "order_id": "ORDER-001",\n  "amount": 50000,\n  "description": "Pembelian produk XYZ",\n  "redirect_url": "https://domain.com/thanks",\n  "webhook_url": "https://domain.com/webhook"\n}\n\n// Response 201\n{\n  "transaction_id": "TXN-abc123",\n  "payment_url": "https://pay.zyfayofficial.com/TXN-abc123",\n  "amount": 50000,\n  "status": "pending",\n  "expired_at": "2025-01-01T12:00:00Z"\n}'},
    {t:"p",v:"Arahkan user ke payment_url. Zyfay Official menangani halaman pilih metode pembayaran."},
  ]},
  {id:"webhook",title:"Webhook",icon:"🔔",content:[
    {t:"p",v:"Zyfay Official mengirim POST ke webhook_url saat status transaksi berubah. Endpoint harus merespons HTTP 200."},
    {t:"code",v:'// Header yang dikirim Zyfay Official:\nX-Zyfay-Signature: sha256=HMAC_SHA256(body, WEBHOOK_SECRET)\n\n// Payload:\n{\n  "event": "transaction.completed",\n  "transaction_id": "TXN-abc123",\n  "order_id": "ORDER-001",\n  "amount": 50000,\n  "status": "completed",\n  "payment_method": "qris",\n  "timestamp": "2025-01-01T11:58:00Z"\n}'},
    {t:"info",v:"Selalu verifikasi signature webhook menggunakan Webhook Secret dari API Settings sebelum memproses."},
    {t:"p",v:"Status: pending → completed | expired | canceled"},
  ]},
  {id:"status",title:"Cek Status",icon:"🔍",content:[
    {t:"code",v:'GET https://api.zyfayofficial.com/v1/transactions/{transaction_id}\n\n// Response\n{\n  "transaction_id": "TXN-abc123",\n  "order_id": "ORDER-001",\n  "amount": 50000,\n  "status": "completed",\n  "payment_method": "qris",\n  "paid_at": "2025-01-01T11:58:00Z"\n}'},
  ]},
  {id:"sandbox",title:"Sandbox",icon:"🧪",content:[
    {t:"p",v:"Mode Sandbox untuk testing tanpa uang nyata. Aktifkan di sidebar dashboard."},
    {t:"code",v:'POST https://api.zyfayofficial.com/v1/sandbox/simulate\n\n{\n  "transaction_id": "TXN-abc123",\n  "status": "completed"  // atau "expired"\n}'},
    {t:"info",v:"Minimum amount simulasi: Rp 1.000. Semua transaksi sandbox tidak diproses secara nyata."},
  ]},
  {id:"ip",title:"IP Whitelist",icon:"🛡️",content:[
    {t:"p",v:"Batasi akses API hanya dari IP server tertentu untuk keamanan ekstra."},
    {t:"steps",v:["Buka API Settings di dashboard","Masukkan IP server di kolom IP Whitelist","Pisahkan beberapa IP dengan koma: 1.2.3.4, 5.6.7.8","Klik Simpan — berlaku dalam 60 detik"]},
    {t:"info",v:"Jika IP Whitelist aktif, request dari IP lain akan ditolak dengan error 403 Forbidden."},
  ]},
  {id:"metode",title:"Metode Pembayaran",icon:"💰",content:[
    {t:"ul",v:["QRIS — GoPay, OVO, Dana, ShopeePay, semua bank","Virtual Account — BCA, BNI, BRI, Mandiri, Permata, BSI, CIMB","E-Wallet — GoPay, ShopeePay langsung","Ritel — Alfamart, Indomaret"]},
    {t:"p",v:"Biaya transaksi dapat dilihat di halaman pricing Zyfay Official."},
  ]},
];

function DocBlock({b}:{b:{t:string;v:string|string[]}}) {
  const base:React.CSSProperties={fontSize:"13px",color:"rgba(255,255,255,0.75)",lineHeight:"1.75",margin:"0 0 10px"};
  if(b.t==="p") return <p style={base}>{b.v as string}</p>;
  if(b.t==="code") return <pre style={{...base,background:"rgba(0,0,0,0.45)",borderRadius:"8px",padding:"13px",fontFamily:"monospace",overflowX:"auto",whiteSpace:"pre-wrap",wordBreak:"break-word",border:"1px solid rgba(255,255,255,0.07)",lineHeight:"1.65"}}>{b.v as string}</pre>;
  if(b.t==="ul") return <ul style={{...base,paddingLeft:"18px"}}>{(b.v as string[]).map((x,i)=><li key={i} style={{marginBottom:"4px"}}>{x}</li>)}</ul>;
  if(b.t==="steps") return <ol style={{...base,paddingLeft:"18px"}}>{(b.v as string[]).map((x,i)=><li key={i} style={{marginBottom:"6px"}}>{x}</li>)}</ol>;
  if(b.t==="info") return <div style={{background:"rgba(108,126,225,0.15)",border:"1px solid rgba(108,126,225,0.3)",borderRadius:"8px",padding:"10px 14px",marginBottom:"10px",fontSize:"12px",color:"#c7d2fe",lineHeight:"1.6"}}>ℹ️ {b.v as string}</div>;
  return null;
}

function Docs() {
  const [active,setActive]=useState("mulai");
  const sec=DOCS.find(d=>d.id===active)!;
  return(
    <div>
      <h2 style={ST.title}>Dokumentasi</h2>
      <p style={{fontSize:"11px",color:"rgba(255,255,255,0.3)",margin:"-10px 0 14px",fontWeight:600,letterSpacing:"0.07em",textTransform:"uppercase"}}>Zyfay Official API</p>
      <div style={{display:"flex",gap:"12px"}}>
        <nav style={{width:"130px",flexShrink:0,display:"flex",flexDirection:"column",gap:"1px"}}>
          {DOCS.map(d=>(
            <button key={d.id} onClick={()=>setActive(d.id)} style={{padding:"7px 10px",border:"none",borderLeft:active===d.id?"2px solid #6c7ee1":"2px solid transparent",background:active===d.id?"rgba(108,126,225,0.18)":"transparent",color:active===d.id?"#c7d2fe":"rgba(255,255,255,0.4)",cursor:"pointer",textAlign:"left",fontSize:"12px",fontWeight:active===d.id?600:400,borderRadius:"0 6px 6px 0",display:"flex",alignItems:"center",gap:"6px"}}>
              <span style={{fontSize:"10px"}}>{d.icon}</span>{d.title}
            </button>
          ))}
        </nav>
        <div style={{flex:1,background:"rgba(255,255,255,0.05)",borderRadius:"12px",padding:"16px 18px",border:"1px solid rgba(255,255,255,0.08)",maxHeight:"58vh",overflowY:"auto"}}>
          <div style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"14px"}}>
            <span style={{fontSize:"18px"}}>{sec.icon}</span>
            <h3 style={{margin:0,fontSize:"15px",fontWeight:700,color:"#fff"}}>{sec.title}</h3>
          </div>
          {sec.content.map((b,i)=><DocBlock key={i} b={b as {t:string;v:string|string[]}}/>)}
        </div>
      </div>
    </div>
  );
...
function ApiSettings() {
  ...
  const [apiKey,setApiKey]=useState("zyfayofficial-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
  const [webhookSecret,setWebhookSecret]=useState("whsec-xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx");
  const [merchantId]=useState("MERCH-"+Math.random().toString(36).slice(2,10).toUpperCase());
  ...
  const generateKey=(prefix:string,setter:React.Dispatch<React.SetStateAction<string>>)=>{
  ...
  const row=(label:string,val:string,show:boolean,setShow:(v:boolean)=>void,copyLabel:string,onGen:()=>void)=>(
    <div>
      <label style={ST.lbl}>{label}</label>
      <div style={{display:"flex",gap:"6px",marginTop:"6px"}}>
        <input type={show?"text":"password"} value={val} readOnly style={{...ST.inp,marginTop:0,flex:1,fontFamily:"monospace",fontSize:"12px"}}/>
        <button onClick={()=>setShow(!show)} style={secBtn}>{show?"Sembunyikan":"Tampilkan"}</button>
        <button onClick={()=>copy(val,copyLabel)} style={secBtn}>{copied===copyLabel?"✓":"Salin"}</button>
        <button onClick={onGen} style={secBtn} title="Generate ulang">↻</button>
      </div>
    </div>
  );
  ...
  return(
    <div>
      <h2 style={ST.title}>API Settings</h2>
      <p style={{color:"rgba(255,255,255,0.4)",fontSize:"13px",margin:"-8px 0 18px"}}>Kelola kredensial & keamanan API Zyfay Official</p>
      <div style={{display:"flex",flexDirection:"column",gap:"14px",maxWidth:"520px"}}>

        {/* Kredensial */}
        <div style={ST.card}>
          <p style={secTitle}>Kredensial API</p>
          {row("API Key","zyfayofficial-"+apiKey.replace("zyfayofficial-",""),showKey,setShowKey,"apikey",()=>generateKey("zyfayofficial-",setApiKey))}
          <div style={{marginTop:"14px"}}>
            <label style={ST.lbl}>Merchant ID</label>
            <div style={{display:"flex",gap:"6px",marginTop:"6px"}}>
              <input type="text" value={merchantId} readOnly style={{...ST.inp,marginTop:0,flex:1,fontFamily:"monospace",fontSize:"12px"}}/>
              <button onClick={()=>copy(merchantId,"merchant")} style={secBtn}>{copied==="merchant"?"✓":"Salin"}</button>
            </div>
          </div>
        </div>
        ...
        {/* Webhook */}
        <div style={ST.card}>
          <p style={secTitle}>Webhook</p>
          <label style={ST.lbl}>Webhook URL<input type="url" placeholder="https://domain.com/api/webhook" value={webhookUrl} onChange={e=>setWebhookUrl(e.target.value)} style={ST.inp}/></label>
          <div style={{marginTop:"14px"}}>
            {row("Webhook Secret","whsec-"+webhookSecret.replace("whsec-",""),showSecret,setShowSecret,"secret",()=>generateKey("whsec-",setWebhookSecret))}
          </div>
          <div style={{marginTop:"10px",background:"rgba(108,126,225,0.12)",borderRadius:"8px",padding:"10px 12px",fontSize:"12px",color:"#c7d2fe",border:"1px solid rgba(108,126,225,0.25)"}}>
            ℹ️ Verifikasi setiap webhook menggunakan HMAC-SHA256 dengan Webhook Secret untuk memastikan keaslian request.
          </div>
        </div>

        {/* IP Whitelist */}
        <div style={ST.card}>
          <p style={secTitle}>IP Whitelist</p>
          <label style={ST.lbl}>
            IP yang diizinkan (pisahkan dengan koma)
            <input type="text" placeholder="1.2.3.4, 5.6.7.8" value={ipWhitelist} onChange={e=>setIpWhitelist(e.target.value)} style={ST.inp}/>
          </label>
          <p style={{fontSize:"12px",color:"rgba(255,255,255,0.35)",margin:"8px 0 0"}}>Kosongkan untuk tidak membatasi IP. Request dari IP lain akan ditolak.</p>
        </div>

        <button onClick={save} disabled={saving} style={{...ST.btn,padding:"12px",borderRadius:"10px",background:saved?"#22c55e":undefined,transition:"background 0.3s"}}>
...
const secBtn:React.CSSProperties={background:"rgba(255,255,255,0.07)",border:"1px solid rgba(255,255,255,0.12)",color:"rgba(255,255,255,0.65)",borderRadius:"8px",padding:"0 10px",cursor:"pointer",fontSize:"11px",fontWeight:600,whiteSpace:"nowrap",height:"40px"};
const secTitle:React.CSSProperties={margin:"0 0 12px",fontSize:"11px",fontWeight:700,color:"rgba(255,255,255,0.5)",letterSpacing:"0.07em",textTransform:"uppercase"};
...
export default function Dashboard() {
  ...
  const [bgTheme,setBgTheme]=useState(THEMES.indigo);
  ...
  return(
    <div style={{minHeight:"100vh",background:bgTheme,color:"#fff",fontFamily:"'Inter',sans-serif"}}>
      ...
      {/* Logout confirm */}
      {showLogout&&(
        <div style={{position:"fixed",inset:0,zIndex:200,background:"rgba(0,0,0,0.5)",backdropFilter:"blur(4px)",display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setShowLogout(false)}>
          <div style={{background:"#1a1b2e",borderRadius:"16px",padding:"28px 24px",width:"300px",border:"1px solid rgba(255,255,255,0.1)",boxShadow:"0 20px 60px rgba(0,0,0,0.6)"}} onClick={e=>e.stopPropagation()}>
            <h3 style={{margin:"0 0 8px",fontSize:"16px",fontWeight:700,color:"#fff"}}>Keluar dari akun?</h3>
            <p style={{margin:"0 0 20px",fontSize:"13px",color:"rgba(255,255,255,0.5)"}}>Sesi Anda akan diakhiri dan perlu login ulang.</p>
            <div style={{display:"flex",gap:"8px"}}>
              <button onClick={()=>setShowLogout(false)} style={{flex:1,padding:"10px",borderRadius:"8px",border:"1px solid rgba(255,255,255,0.12)",background:"transparent",color:"rgba(255,255,255,0.6)",cursor:"pointer",fontWeight:600,fontSize:"13px"}}>Batal</button>
              <button onClick={logout} style={{flex:1,padding:"10px",borderRadius:"8px",border:"none",background:"#ef4444",color:"#fff",cursor:"pointer",fontWeight:700,fontSize:"13px"}}>Ya, Keluar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* SIDEBAR */}
      <aside style={{position:"fixed",top:0,left:open?"0":"-270px",width:"248px",height:"100vh",background:"#fafafa",padding:"20px 14px",transition:"left 0.28s",zIndex:10,boxShadow:"2px 0 16px rgba(0,0,0,0.15)",overflowY:"auto",display:"flex",flexDirection:"column"}}>
        <div style={{marginBottom:"22px"}}><h2 style={{margin:0,color:"#3e4a89",fontSize:"19px",fontWeight:800,letterSpacing:"-0.01em"}}>Zyfayment</h2></div>
        ...
        <nav style={{display:"flex",flexDirection:"column",gap:"1px",flex:1}}>
          {NAV.map(n=>nb(n.id as Page,n.label,n.icon))}
          ...
          {SETTINGS.map(n=>nb(n.id as Page,n.label,n.icon))}
        </nav>
        {/* Logout di bawah sidebar */}
        <button onClick={()=>setShowLogout(true)} style={{display:"flex",alignItems:"center",gap:"8px",background:"transparent",border:"1px solid rgba(239,68,68,0.3)",borderRadius:"8px",padding:"9px 10px",color:"#ef4444",fontWeight:600,fontSize:"13px",cursor:"pointer",width:"100%",marginTop:"12px"}}>
          <IC.LogOut s={15} c="#ef4444"/>Keluar
        </button>
      </aside>
      ...
      <main style={{padding:"18px 16px"}}>
        <header style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <button onClick={()=>setOpen(true)} style={{background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.1)",color:"#fff",cursor:"pointer",borderRadius:"10px",width:"36px",height:"36px",display:"flex",alignItems:"center",justifyContent:"center"}}>
            <IC.Menu s={19}/>
          </button>
          <div style={{display:"flex",gap:"8px",alignItems:"center"}}>
            <button onClick={()=>setShowNotif(v=>!v)} style={{position:"relative",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.1)",color:"#fff",borderRadius:"10px",width:"36px",height:"36px",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <IC.Bell s={17}/>
              {unread>0&&<span style={{position:"absolute",top:"5px",right:"5px",width:"7px",height:"7px",borderRadius:"50%",background:"#ef4444",border:"2px solid rgba(0,0,0,0.3)"}}/>}
            </button>
