    ...
    <div style={styles.page}>
      ...
      <aside style={{...styles.sidebar, left: isOpen ? "0" : "-280px"}}>
        ...
        <nav style={styles.nav}>
          {[{n:"Dashboard", p:"/dashboard"}, {n:"Sandbox Panel", p:"/sandbox", s:true}, {n:"Transaksi", p:"/transaksi"}, {n:"Dompet", p:"/dompet"}, {n:"Tarik Dana", p:"/tarik"}].map(m => (!m.s || mode === "SANDBOX") && <button key={m.n} style={styles.link} onClick={() => router.push(m.p)}>{m.n}</button>)}
          <div style={styles.divider}>API & PENGATURAN</div>
          {[{n:"API Settings", p:"/api"}, {n:"Dokumentasi API", p:"/docs"}, {n:"Profil Saya", p:"/profile"}].map(m => <button key={m.n} style={styles.link} onClick={() => router.push(m.p)}>{m.n}</button>)}
        </nav>
      </aside>
        ...
        <header style={styles.header}>
          ...
          <div style={styles.headerRight}>
            <div style={{position: 'relative'}}>
              <button onClick={() => setShowNotif(!showNotif)} style={styles.iconBtn}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9"/><path d="M13.73 21a2 2 0 0 1-3.46 0"/></svg></button>
              {showNotif && (
                <div style={styles.popup}>
                  <p style={{margin: '0 0 10px', fontSize: '13px'}}>Tidak ada info terbaru</p>
                  <div style={{display:'flex', gap:'5px'}}><button style={styles.btnSm}>Hapus</button><button style={styles.btnSm}>Baca</button></div>
                </div>
              )}
            </div>
            <button onClick={() => router.push("/profile")} style={styles.avatar}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#333" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></button>
          </div>
        </header>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  brand: { fontWeight: 800, fontSize: "20px", color: "#3e4a89", marginBottom: "30px" },
  ...
  link: { background: "none", border: "none", padding: "12px 0", cursor: "pointer", fontSize: "14px", textAlign: "left", color: "#333", fontWeight: 500 },
  divider: { fontSize: "11px", color: "#999", margin: "20px 0 10px", fontWeight: "bold" },
  main: { padding: "20px" },
  ...
  popup: { position: "absolute", top: "45px", right: "50px", background: "#fff", color: "#333", padding: "15px", borderRadius: "8px", boxShadow: "0 5px 15px rgba(0,0,0,0.3)", width: "180px" },
  btnSm: { fontSize: "11px", padding: "4px 8px", cursor: "pointer" },
